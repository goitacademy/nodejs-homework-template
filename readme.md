...
обязательных полей, возвращает json с ключом
`{"message": "missing required name field"}` и `статусом 400` Если с body все
хорошо, добавляет уникальный идентификатор в объект контакта Вызывает функцию
`addContact(body)` для сохранения контакта в файле contacts.json По результату
работы функции возвращает объект с добавленным id `{id, name, email, phone}` и
`статусом 201`
...
**Для маршрутов, что принимают данные `(POST, PUT, PATCH)`, продумана проверка
