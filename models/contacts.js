...
const getContactById = async (contactId) => {
  try {
    ...
    const findContactById = contacts.find(
      (c) =>
        c.id.toString().toLowerCase() === contactId.toString().toLowerCase()
    );
...
const removeContact = async (contactId) => {
  try {
    ...
    const delContact = contacts.filter(
      (contact) =>
        contact.id.toLowerCase().toString() ===
        contactId.toLowerCase().toString()
    );
...
const updateContact = async (id, body) => {
  try {
    ...
    const idx = contacts.findIndex(
      (c) => c.id.toLowerCase().toString() === id.toLowerCase().toString()
    );
