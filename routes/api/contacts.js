...
const { updateSchema, addSchema } = require("../../models/schemas");
...
router.post("/", validation(addSchema), async (req, res, next) => {
  try {
...
router.put("/:contactId", validation(updateSchema), async (req, res, next) => {
  try {
    const { contactId } = req.params;
