...
const verifyEmail = catchAsync(async (req, res) => {
  ...
  const mess = await userServices.verifyEmail({ verificationToken });

  res.status(200).json(mess);
});

const forwardVerification = catchAsync(async (req, res) => {
  const { email } = req.body;

  const mess = await userServices.forwardVerification({ email });
